...
export class RedirectLinksPrintAssetsController {
  constructor(
  ...
  async buildPrintAssets(@Body() body: RequestBody, @Res() res: Response) {
    ...
    for (const id of ids) {
      ...
      const qrValue = this.redirectLinksService.getPublicUrl(link);
      ...
      cards.push(`
        <div style="border:1px solid #ddd;border-radius:16px;padding:20px;margin:12px;display:inline-block;width:320px;vertical-align:top;font-family:Arial,sans-serif;">
          ...
          <div style="font-size:20px;font-weight:700;margin-bottom:8px;">${link.title ?? link.slug}</div>
          <div style="font-size:13px;color:#444;margin-bottom:12px;">Slug: ${link.slug}</div>
          <img src="${dataUrl}" alt="QR ${link.slug}" style="width:220px;height:220px;display:block;margin:0 auto 14px auto;" />
    ...
    const html = `
      ...
      <html>
        <head>
          <meta charset="utf-8" />
          <title>Redirect Links Print Assets</title>
        </head>
